...
import { alert, getEnv, log, showTitleModal } from './util';
import {
  autoRangeByRecent,
  calcLargerFrame,
...
Key.on('6', config.MASH_CTRL, () => callApp('ChatWise'));
Key.on('7', config.MASH_CTRL, () => callApp('Ghostty'));
...
Key.on('r', config.MASH_CTRL, () =>
  callApp(
    '/Applications/Antding.app/Contents/Frameworks/Tblive.app/Contents/MacOS/Tblive',
...
Key.on(',', config.MASH_CTRL, () => callApp('Yuque'));
